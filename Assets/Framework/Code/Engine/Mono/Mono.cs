...
namespace Jape
{
	...
	public partial class Mono : MonoBehaviour
    {
        ...
        /// <summary>
        /// Called before Init() when first created, called before Enabled() when set to active, called once in lifetime
        ...
        internal virtual void OnGUI() { Draw(); }
        ...
        internal IEnumerable<AttributeField<T>> GetAttributeMembers<T>(bool inherit = false) where T : Attribute
        {
            return AttributeField<T>.FromTargets(GetType().GetFields(), inherit);
        }
