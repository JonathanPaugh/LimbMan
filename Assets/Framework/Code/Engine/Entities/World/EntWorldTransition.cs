...
namespace Jape
{
    ...
    {
        protected override Texture Icon => GetIcon("IconTransition");
        ...
        protected override void Default() { loadingScreen = Game.Settings<SceneSettings>().defaultLoadingScreen; }
        ...
        private void OnDrawGizmos()
        {
            ...
            GUIContent labelContent = new GUIContent($" {map.GetSceneName()} - {GetTargets().FirstOrDefault(t => t.Value == target).Text}");
            ...
            if (map.IsSet())
            {
                Handles.Label(transform.position, iconContent, new GUIStyle().Offset(new Vector2(-4, yOffset - 20)).Padding(new RectOffset(-24, -24, -24, -24)));
                Handles.Label(transform.position, labelContent, style.Offset(new Vector2(xOffset, yOffset)));
            }
        }

        #endif

        private static IList<ValueDropdownItem<Map>> LoadingScreenDropdown() { return Game.Settings<SceneSettings>().LoadingScreenDropdown(); }
    }
}