...
namespace JapeEditor
{
    public class RouterWindow : Window
    {
        ...
        private static GUIStyle TextStyle => new GUIStyle(GUI.skin.textField);
        private static GUIStyle DropdownStyle => new GUIStyle(GUI.skin.GetStyle("Popup"));
        private static GUIStyle MiniButtonStyle => new GUIStyle(GUI.skin.button.Padding(new RectOffset(1, 1, 1, 1)));
        ...
        {
            List<Entity.Routing> deleted = new List<Entity.Routing>();
        ...
        private bool DrawSelection(int index)
        {
            ...
            Rect selectionPosition = new Rect(0, Space + (index * (RoutingHeight() + 2)), WindowWidth(), RoutingHeight() + 1);
            Rect deletePosition = new Rect(position.width - 26, Space + (index * (RoutingHeight() + 2)), 16, RoutingHeight());
            
            if (GUI.Button(deletePosition, GUIContent.none)) { return true; }
            GUI.Label(deletePosition, Database.GetAsset("IconDelete").Load<Texture>(), GUIStyle.none);
        ...
        private void DrawFields()
        {
            ...
            void DrawParameters()
            {
                ...
                void Smart()
                {
                    ...
                    List<string> parameterInputs = new List<string>();
