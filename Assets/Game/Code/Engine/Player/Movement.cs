...
namespace Game
{
    public class Movement : Element
    {
        ...
        private void Input()
        {
            if (input.useTouchControls)
            {
                inputHorizontal = input.TouchControls.Axis.Horizontal;
                inputVertical = input.TouchControls.Axis.Vertical;
                inputJump = input.TouchControls.JumpStream();
                inputDodge = input.TouchControls.DodgeStream();
            }
            else
            {
                inputHorizontal = input.HorizontalStream() ?? 0;
                inputVertical = input.VerticalStream() ?? 0;
